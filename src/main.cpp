...
const string strMessageMagic = "APCDCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
   ...
   if (pindexBest->nHeight <= 1)
      ...
      {
        int64_t nSubsidy = .001 * COIN;
        return nSubsidy + nFees;
      }
   
    else if (pindexBest->nHeight <= 10020)
      {
        int64_t nSubsidy = 100 * COIN;
        return nSubsidy + nFees;
      }
   
    else if (pindexBest->nHeight <= 20020)
      {
        int64_t nSubsidy = 50 * COIN;
        return nSubsidy + nFees;
      }
   
    else if (pindexBest->nHeight <= 30020)
      {
        int64_t nSubsidy = 25 * COIN;
        return nSubsidy + nFees;
      }
   
    else if (pindexBest->nHeight <= 105020)
      {
        int64_t nSubsidy = 10 * COIN;
        return nSubsidy + nFees;
      }
   
    else if (pindexBest->nHeight <= 1005020)
      {
...
void ThreadScriptCheck(void*) {
    ...
    RenameThread("APCDCoin-scriptch");
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in APCDCoin
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "APCDCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "The 'backdoor Roth IRA' may be ending CNN 11-30-2021 12:01AM PST";
        CTransaction txNew;
        txNew.nTime = 1638259306;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print        
        block.print();
        assert(block.hashMerkleRoot == uint256("0xa89c46bb1eeb50fc37a143eec575c6f86b2e544cd23275318e0878653e7cf795"));
