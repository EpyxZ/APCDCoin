...
static const char *strDNSSeed[][2] = {
    {"162.220.51.41", "162.220.51.41"},
    {"88.202.187.230", "88.202.187.230"},
    {"185.225.210.103", "185.225.210.103"},
    {"172.98.73.33", "172.98.73.33"},
    {"31.24.227.19", "31.24.227.19"},
};

void ThreadDNSAddressSeed(void* parg)
{
    // Make this thread recognisable as the DNS seeding thread
    RenameThread("APCDCoin-dnsseed");
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. APCDCoin is probably already running."), addrBind.ToString().c_str());
        else
