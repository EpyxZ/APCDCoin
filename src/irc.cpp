...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #APCDCoinTEST2\r");
            Send(hSocket, "WHO #APCDCoinTEST2\r");
        } else {
            // randomly join #APCDCoin00-#APCDCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #APCDCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #APCDCoin%02d\r", channel_number).c_str());
        }
